...
public class Drive extends Subsystem {
    ...
    // 1 - Practice Bot
    // 2 - Main Robot
    private static final double ROBOT_CONFIGURATION = 1;

    // Wheel base lengthes are measured from where wheels touch the ground
    ...
    public void swerveDrive(double strafe, double forward, double omega) {
        ...
        if (ROBOT_CONFIGURATION == 1) {
            // Practice Robot Configuration
            double angleLF = angle(B, D) - 112;
            double angleLR = angle(A, D) - 80.5; // - 85
            double angleRF = angle(B, C) + 97.5; // Increasing turns to the right
            double angleRR = angle(A, C) + 22; // + 14
        }
        else if (ROBOT_CONFIGURATION == 2) {
            ...
            System.out.println("Default motor angles in use. Please specify ROBOT_CONFIGURATION (1 - Practice Bot; 2 - Main Bot).");
