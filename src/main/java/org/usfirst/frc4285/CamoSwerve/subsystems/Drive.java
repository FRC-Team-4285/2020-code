...
public class Drive extends Subsystem {
    ...
    public void swerveDrive(double strafe, double forward, double omega) {
        ...
        if (robot_config == 1) {
            ...
            angleLR = angle(A, D) - 80.5; // - 85
            angleRF = angle(B, C) + 97.5; // Increasing turns to the right
            angleRR = angle(A, C) + 22; // + 14
        }
        else if (robot_config == 2) {
            // Main Robot Configuration
            angleLF = angle(B, D) - 91;
            angleLR = angle(A, D) + 93;
            angleRF = angle(B, C) - 38;
            angleRR = angle(A, C) + 84;
        }
